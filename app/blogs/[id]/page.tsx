...
async function Blog({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: BlogDetails } = await supabase.from('blogs').select('*').eq('id', blogId).single<Blog>();
  const { data } = await supabase.from('blog_scetions').select('*').eq('blog_id', blogId).order('order', { ascending: true }).returns<Section[]>();
  const createdAtDate = new Date(BlogDetails?.created_at || '2025-06-12').toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });

  return (
    <div className='mt-8 md:mt-16 w-full flex flex-col justify-start items-start gap-y-6 md:gap-y-7'>
      <Link href={'/blogs'}>
      ...
      <div className='w-full h-56 md:h-100 rounded-2xl border-2 border-border overflow-hidden'>
      ...
      <div className='mt-10 mb-10 w-full'>
        <div className='text-foreground text-sm md:text-xl px-4 py-2 border-2 border-border w-fit bg-card rounded-xl'>
          Written with ❤️ by <span className='text-foreground font-semibold'>Akarsh Jha</span>
        </div>
      </div>
    </div>
  )
